...
parser_error_t tx_traverse(int16_t root_token_index, uint8_t *numChunks) {
  ...
  const parser_error_t err =
      tx_traverse_find(root_token_index, &ret_value_token_index);

  if (err != parser_ok) {
    return err;
  }

  return tx_getToken(ret_value_token_index, parser_tx_obj.tx_json.query.out_val,
...
TEST(TxParse, Tx_Traverse) {
  ...
  parser_error_t err =
      JSON_PARSE(&parser_tx_obj.tx_json.json, parser_tx_obj.tx_json.tx);

  ASSERT_EQ(err, parser_ok);
  // Check some tokens
  ASSERT_EQ(parser_tx_obj.tx_json.json.numberOfTokens, 7)
      << "It should contain 7 = 1 (dict) + 6 (key+value)";
  ASSERT_EQ(parser_tx_obj.tx_json.json.tokens[0].start, 0);
  ASSERT_EQ(parser_tx_obj.tx_json.json.tokens[0].end, 46);
  ASSERT_EQ(parser_tx_obj.tx_json.json.tokens[0].size, 3)
      << "size should be 3 = 3 key/values contained in the dict";
  ...
  parser_tx_obj.tx_json.query.item_index = 1;
  err = tx_traverse(0, &numChunks);
  EXPECT_EQ(err, parser_display_page_out_of_range)
...
TEST(TxParse, OutOfBoundsSmall) {
  ...
  err = tx_traverse(0, &numChunks);
  EXPECT_EQ(err, parser_display_page_out_of_range)
      << "This call should have resulted in a display out of range";
