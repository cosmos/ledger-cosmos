...
TEST(TxValidationTest, MissingAccountNumber) {
  ...
  EXPECT_EQ(err, parser_json_missing_account_number)
      << "Validation failed, error: " << parser_getErrorDescription(err);
}
...
TEST(TxValidationTest, MissingFee) {
  ...
  err = tx_validate(&json);
  EXPECT_EQ(err, parser_json_missing_fee)
      << "Validation failed, error: " << parser_getErrorDescription(err);
}
...
TEST(TxValidationTest, MissingMsgs) {
  ...
  err = tx_validate(&json);
  EXPECT_EQ(err, parser_json_missing_msgs)
      << "Validation failed, error: " << parser_getErrorDescription(err);
}
...
TEST(TxValidationTest, MissingSequence) {
  ...
  err = tx_validate(&json);
  EXPECT_EQ(err, parser_json_missing_sequence)
      << "Validation failed, error: " << parser_getErrorDescription(err);
}
...
TEST(TxValidationTest, Spaces_InTheMiddle) {
  ...
  err = tx_validate(&json);
  EXPECT_EQ(err, parser_json_contains_whitespace)
      << "Validation failed, error: " << parser_getErrorDescription(err);
}
...
TEST(TxValidationTest, NotSortedDictionary_FirstElement) {
  ...
  err = tx_validate(&json);
  EXPECT_EQ(err, parser_json_is_not_sorted)
      << "Validation failed, error: " << parser_getErrorDescription(err);
}
...
// This json has been taken directly from goclient which uses cosmos to
// serialize a simple tx This test is currently failing the validation. We are
// reviewing the validation code and cosmos serialization to find the culprit.
