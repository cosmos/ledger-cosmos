...
bool copy_transaction_parameters(
  ...
  // Check and copy memo
  size_t memo_len = strnlen(
      sign_transaction_params->destination_address_extra_id, sizeof(memo));
  ...
  // Check and copy fees
  memcpy(fees, sign_transaction_params->fee_amount,
...
/*
 * This function verifies that a received transaction follows the expected
 * format based on the current application mode (expert or normal). The
 * verification process includes checking the number of items in the transaction
 * and validating that the items at its respective display index matches the
 * expected content. If any item does not meet the expected criteria, the
 ...
 */
parser_error_t parser_msg_send(parser_context_t *ctx_parsed_tx,
                               uint8_t displayIdx, uint8_t pageIdx,
                               uint8_t pageCount) {
  ...
  CHECK_PARSER_ERR(parser_getItem(ctx_parsed_tx, displayIdx, tmpKey,
                                  sizeof(tmpKey), tmpValue, sizeof(tmpValue),
                                  pageIdx, &pageCount))
  ...
  zxerr_t zxerr = format_amount(G_swap_state.amount, G_swap_state.amount_length,
                                tmp_amount, sizeof(tmp_amount), chain_index);
  if (zxerr != zxerr_ok) {
  ...
  CHECK_PARSER_ERR(parser_getItem(ctx_parsed_tx, displayIdx, tmpKey,
                                  sizeof(tmpKey), tmpValue, sizeof(tmpValue),
                                  pageIdx, &pageCount))
  if (strcmp(tmpKey, "To") != 0 ||
      strcmp(tmpValue, G_swap_state.destination_address) != 0) {
    ZEMU_LOGF(200,
              "Wrong swap tx destination address ('%s', should be : '%s').\n",
  ...
  // Check fees
  zxerr = format_amount(G_swap_state.fees, G_swap_state.fees_length, tmp_amount,
 ...
 *   matches the expected count for the current mode.
 */
parser_error_t parser_simple_transfer(parser_context_t *ctx_parsed_tx,
                                      uint8_t displayIdx, uint8_t pageIdx,
                                      uint8_t pageCount) {
  if (ctx_parsed_tx == NULL) {
    return parser_unexpected_error;
  ...
  CHECK_PARSER_ERR(parser_getItem(ctx_parsed_tx, displayIdx, tmpKey,
                                  sizeof(tmpKey), tmpValue, sizeof(tmpValue),
                                  pageIdx, &pageCount))
  if (strcmp(tmpKey, "Source Coins") != 0 ||
      strcmp(tmpValue, tmp_amount) != 0) {
  ...
  CHECK_PARSER_ERR(parser_getItem(ctx_parsed_tx, displayIdx, tmpKey,
                                  sizeof(tmpKey), tmpValue, sizeof(tmpValue),
                                  pageIdx, &pageCount))
  if (strcmp(tmpKey, "Dest Address") != 0 ||
      strcmp(tmpValue, G_swap_state.destination_address) != 0) {
    ZEMU_LOGF(200,
              "Wrong swap tx destination address ('%s', should be : '%s').\n",
              tmpValue, G_swap_state.destination_address);
...
parser_error_t check_swap_conditions(parser_context_t *ctx_parsed_tx) {
  ...
  parser_error_t err =
      parser_simple_transfer(ctx_parsed_tx, displayIdx, pageIdx, pageCount);
