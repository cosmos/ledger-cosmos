...
void validate_testcase(const testcase_t &tc) {
  ...
  ASSERT_EQ(parser_getErrorDescription(err), tc.parsingErr)
      << "Parsing error mismatch";

  if (err != parser_ok)
    return;

  err = parser_validate(&ctx);
  EXPECT_EQ(parser_getErrorDescription(err), tc.validationErr)
