...
namespace {
    ...
    int get_token_good_message(char* name, int level)
    {
        auto transaction = R"({"alt_bytes":null,"chain_id":"test-chain-1","fee_bytes":{"amount":[{"amount":5,"denom":"photon"}],"gas":10000},"msg_bytes":{"inputs":[{"address":"696E707574","coins":[{"amount":10,"denom":"atom"}]}],"outputs":[{"address":"6F7574707574","coins":[{"amount":10,"denom":"atom"}]}]},"sequences":[1]})";
        char errorMsg[20];
    ...
    TEST(TransactionParserTest, get_token_alt_bytes) {
        auto result = get_token_good_message("alt_bytes", 1);
        EXPECT_EQ(result, 1) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    TEST(TransactionParserTest, get_token_chain_id) {
        auto result = get_token_good_message("chain_id", 1);
        EXPECT_EQ(result, 3) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    TEST(TransactionParserTest, get_token_fee_bytes) {
        auto result = get_token_good_message("fee_bytes", 1);
        EXPECT_EQ(result, 5) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    TEST(TransactionParserTest, get_token_msg_bytes) {
        auto result = get_token_good_message("msg_bytes", 1);
        EXPECT_EQ(result, 14) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    TEST(TransactionParserTest, get_token_sequences) {
        auto result = get_token_good_message("sequences", 1);
        EXPECT_EQ(result, 32) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }

    TEST(TransactionParserTest, get_token_alt_bytes_wrong_level) {
        auto result = get_token_good_message("alt_bytes", 2);
        EXPECT_EQ(result, -1) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    TEST(TransactionParserTest, get_token_chain_id_wrong_level) {
        auto result = get_token_good_message("chain_id", 3);
        EXPECT_EQ(result, -1) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    TEST(TransactionParserTest, get_token_fee_bytes_wrong_level) {
        auto result = get_token_good_message("fee_bytes", 0);
        EXPECT_EQ(result, -1) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    TEST(TransactionParserTest, get_token_msg_bytes_wrong_level) {
        auto result = get_token_good_message("msg_bytes", 4);
        EXPECT_EQ(result, -1) << "Wrong token index, expected: " << 0 << ", received: " << result;
    }
    ...
    TEST(TransactionParserTest, read_fee_offset_0) {
        ...
        parsed_json_t parsed_json;
        json_parse(&parsed_json, transaction);
        int token_index = json_get_token(
                transaction,
                &parsed_json,
                "fee_bytes",
                1);
        ...
        char buffer[20];
        json_read_token(transaction, parsed_json, token, token_index, buffer, sizeof(buffer), 0);
    ...
    TEST(TransactionParserTest, read_fee_offset_20) {
        ...
        char buffer[20];
        json_read_token(transaction, parsed_json, token, token_index, buffer, sizeof(buffer), 20);
