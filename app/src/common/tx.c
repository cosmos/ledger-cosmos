...
const char *tx_parse(tx_type_e type) {
  ...
  tx_obj.tx_type = type;
  uint8_t err = parser_parse(&ctx_parsed_tx, tx_get_buffer(),
                             tx_get_buffer_length(), &tx_obj);
  ...
  if (G_swap_state.called_from_swap) {
    if (G_swap_state.should_exit == 1) {
      ...
      // This panic quit is a failsafe that should never trigger, as the app is
      // supposed to exit after the first send when started in swap mode
      os_sched_exit(-1);
    } else {
